@php
    $storeBackgroundUrl = \App\Models\StoreSetting::assetUrl('landing_background_path', 'assets/brand/landing-background.png');
    $storeBackgroundBlur = max(0, min(24, (int) \App\Models\StoreSetting::value('landing_background_blur', 5)));
@endphp
<!DOCTYPE html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <style>
            ...
            *{box-sizing:border-box} html{scroll-behavior:smooth} body{margin:0;color:var(--ink);font-family:"Manrope",sans-serif}
            body.store-front{position:relative;background:#9ebbc2;isolation:isolate}
            body.store-front::before{content:"";position:fixed;inset:0;z-index:-2;pointer-events:none;background:url('{{ $storeBackgroundUrl }}') center center / cover no-repeat;filter:blur({{ $storeBackgroundBlur }}px) saturate(.92) brightness(.9);transform:scale(1.04)}
            body.store-front::after{content:"";position:fixed;inset:0;z-index:-1;pointer-events:none;background:radial-gradient(circle at top,rgba(255,255,255,.18),transparent 24%),linear-gradient(180deg,rgba(215,232,235,.56) 0%,rgba(170,197,203,.5) 38%,rgba(122,161,169,.46) 100%)}
    ...
    <body class="store-front @yield('body_class')">
        <div class="wrap">
            <header class="topbar">
